...
export const TextMessageBlob: React.FC<Props> = ({ chat, viewingUser, isPreview }) => {
    ...
    const [bottomLabelClasses, setBottomLabelClasses] = useState<Array<string>>([]);

    const assignClasses = useRef(() => {});
    assignClasses.current = () => {
    ...
    return (
        <div className="txt-msg-blob-row">
            <article className={blobClasses.join(' ')}>{chat.msg}</article>
            <label className={bottomLabelClasses.join(' ')}>{isPreview ? 'Sending...' : getFormattedSentDate()}</label>
        </div>
    );
};
