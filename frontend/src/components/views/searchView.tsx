...
export const SearchView: React.FC = () => {
    ...
    const friendTypeActionHandler = {
        ...
        FRIEND: (_: number, __: string): Promise<void> => new Promise<void>((resolve, reject) => {}),
    };

    const searchFriendsBySearchTerm = async (newSearchTerm: string): Promise<void> => {
        setSearchTerm(searchTerm);
        try {
            const users: SearchUserResponse = await searchUsers(newSearchTerm, localStorage.getItem('authToken'));
    ...
    const getCardTypeByUser = (user: User) => {
        if (searchResults.friendRequestedUsersId.includes(user.id)) return CardType.FRIEND_REQUEST;
        else if (searchResults.friendUsersId.includes(user.id)) return CardType.FRIEND;
        else return CardType.STRANGER;
    };
    ...
    const sendFriendRequestHandler = async (user: User) => {
        ...
        try {
            const res: any = await friendTypeActionHandler[cardType](user.id, authUserToken);

            if (res.status === 'success') searchFriendsBySearchTerm(searchTerm);
        } catch (e) {
            console.log(e);
            clearSession();
        }
